...
class DriveTrain:
    ...
    def __init__(self, data_path, base_model_path):
        ...
        csv_path = data_path + '/' + model_name + '_delay_mitigation' + const.DATA_EXT  # use it for csv file name
        ...
        self.net_model = NetModel(data_path, base_model_path)
    ...
    def _prepare_data(self):
        ...
        samples = list(zip(self.data.image_names, self.data.velocities, self.data.calculated_accelerations, self.data.actions,
                           self.data.actions_future_9, self.data.actions_future_12, self.data.actions_future_15,
    ...
    def _build_model(self, show_summary=True):
        ...
        def _prepare_batch_samples(batch_samples):
            ...
            for image_name, velocity, acceleration, action, action_9, action_12, action_15, action_18, action_21 in batch_samples:
                ...
                if config['num_outputs'] == 6: # for delay metigation --> model: mlp_all
                    actions.append((steering_angle*config['steering_angle_scale'], throttle, brake, throttle_brake))
                    actions_9.append((steering_angle_9*config['steering_angle_scale'], throttle_9, brake_9, throttle_brake_9))
                    actions_12.append((steering_angle_12*config['steering_angle_scale'], throttle_12, brake_12, throttle_brake_12))
                    actions_15.append((steering_angle_15*config['steering_angle_scale'], throttle_15, brake_15, throttle_brake_15))
                    actions_18.append((steering_angle_18*config['steering_angle_scale'], throttle_18, brake_18, throttle_brake_18))
                    actions_21.append((steering_angle_21*config['steering_angle_scale'], throttle_21, brake_21, throttle_brake_21))
                ...
                append, image, steering_angle_9 = _data_augmentation(image, steering_angle_9)
                append, image, steering_angle_12 = _data_augmentation(image, steering_angle_12)
                append, image, steering_angle_15 = _data_augmentation(image, steering_angle_15)
                append, image, steering_angle_18 = _data_augmentation(image, steering_angle_18)
                append, image, steering_angle_21 = _data_augmentation(image, steering_angle_21)
                if append is True:
                    images.append(image)
                    velocities.append(velocity)
                    accelerations.append(acceleration)

                    if config['num_outputs'] == 6:
                        actions.append((steering_angle*config['steering_angle_scale'], throttle, brake, throttle_brake))
                        actions_9.append((steering_angle_9*config['steering_angle_scale'], throttle_9, brake_9, throttle_brake_9))
                        actions_12.append((steering_angle_12*config['steering_angle_scale'], throttle_12, brake_12, throttle_brake_12))
                        actions_15.append((steering_angle_15*config['steering_angle_scale'], throttle_15, brake_15, throttle_brake_15))
            ...
            return images, velocities, accelerations, actions, actions_9, actions_12, actions_15, actions_18, actions_21
            ...
            while True: # Loop forever so the generator never terminates
                ...
                if config['lstm'] is True:
                    ...
                    for offset in range(0, num_samples, batch_size):
                        ...
                        images, velocities, accelerations, actions, actions_9, actions_12, actions_15, actions_18, actions_21 = _prepare_batch_samples(batch_samples)
                        # print('#################')
                        ...
                        # Assuming 'actions' is a list of tuples where each tuple contains (steering, throttle, brake, thr_brk)
                        ...
                        steering_9_true = np.array([action[0] for action in actions_9]).reshape(-1, 1)
                        throttle_brake_9_true = np.array([(action[3]) for action in actions_9]).reshape(-1, 1)

                        steering_12_true = np.array([action[0] for action in actions_12]).reshape(-1, 1)
                        throttle_brake_12_true = np.array([(action[3]) for action in actions_12]).reshape(-1, 1)

                        steering_15_true = np.array([action[0] for action in actions_15]).reshape(-1, 1)
                        throttle_brake_15_true = np.array([(action[3]) for action in actions_15]).reshape(-1, 1)

                        steering_18_true = np.array([action[0] for action in actions_18]).reshape(-1, 1)
                        throttle_brake_18_true = np.array([(action[3]) for action in actions_18]).reshape(-1, 1)

                        steering_21_true = np.array([action[0] for action in actions_21]).reshape(-1, 1)
                        throttle_brake_21_true = np.array([(action[3]) for action in actions_21]).reshape(-1, 1)

                        # Combine the steering and throttle_brake arrays
                        if not config['delay_mitig']:
                            y_train = [steering_true, throttle_brake_true]
                        else:
                            # print("Train for delay mitigation ...")
                            if not config['mlp_all']:
                                if config['delta'] == 9:
                                    y_train = [steering_9_true, throttle_brake_9_true]
                                elif config['delta'] == 12:
                                    y_train = [steering_12_true, throttle_brake_12_true]
                                elif config['delta'] == 15:
                                    y_train = [steering_15_true, throttle_brake_15_true]
                                elif config['delta'] == 18:
                                    y_train = [steering_18_true, throttle_brake_18_true]
                                elif config['delta'] == 21:
                                    y_train = [steering_21_true, throttle_brake_21_true]
                            else:
                                # y_train = [steering_true, steering_9_true, steering_12_true,
                                #            steering_15_true, steering_18_true, steering_21_true]
                                y_train = [steering_9_true, steering_12_true,
                                           steering_15_true, steering_18_true, steering_21_true]
