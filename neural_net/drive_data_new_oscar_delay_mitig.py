...
class DriveData:
    ...
    # for delay mitigation, the csv file already has a header
    ...
    def __init__(self, csv_fname):
        ...
        self.actions_future_9 = []
        self.actions_future_12 = []
        self.actions_future_15 = []
        self.actions_future_18 = []
        self.actions_future_21 = []
        

    def read(self, read = True, show_statistics = True, normalize = True):
        # for delay mitigation we do not need set the other arguments because the csv file already has a header
        ...
        print(self.df.columns.tolist()) # Print column names inferred from the header row
        print('*********************************************************************************************')

        # add a new colum which would have both throttle and brake values 
        if 'brake' in self.df.columns:
            self.df['throttle_brake'] = self.df['throttle'] - self.df['brake']
            # for delay mitigation
            self.df['future_9_throttle_brake'] = self.df['future_9_throttle'] - self.df['future_9_brake']
            self.df['future_12_throttle_brake'] = self.df['future_12_throttle'] - self.df['future_12_brake']
            self.df['future_15_throttle_brake'] = self.df['future_15_throttle'] - self.df['future_15_brake']
            self.df['future_18_throttle_brake'] = self.df['future_18_throttle'] - self.df['future_18_brake']
            self.df['future_21_throttle_brake'] = self.df['future_21_throttle'] - self.df['future_21_brake']
        ############################################
        # show statistics
        if (show_statistics):
            print('\n####### data statistics #########')
        ...
        if (Config.neural_net['normalize_data'] and normalize):
            ...
            num_bins = 35 # it was 50
            samples_per_bin = 2000 # it was 200
            remove_list = []
        ...
        if (read): 
            ...
            # for i in bar(range(10)):
                ...
                if Config.data_collection['brake'] is True:
                    ...
                    self.actions_future_9.append((float(self.df.loc[i]['future_9_steering_angle']),
                                            float(self.df.loc[i]['future_9_throttle']), 
                                            float(self.df.loc[i]['future_9_brake']),
                                            float(self.df.loc[i]['future_9_throttle_brake'])))
                    self.actions_future_12.append((float(self.df.loc[i]['future_12_steering_angle']),
                                            float(self.df.loc[i]['future_12_throttle']), 
                                            float(self.df.loc[i]['future_12_brake']),
                                            float(self.df.loc[i]['future_12_throttle_brake'])))
                    self.actions_future_15.append((float(self.df.loc[i]['future_15_steering_angle']),
                                            float(self.df.loc[i]['future_15_throttle']), 
                                            float(self.df.loc[i]['future_15_brake']),
                                            float(self.df.loc[i]['future_15_throttle_brake'])))
                    self.actions_future_18.append((float(self.df.loc[i]['future_18_steering_angle']),
                                            float(self.df.loc[i]['future_18_throttle']), 
                                            float(self.df.loc[i]['future_18_brake']),
                                            float(self.df.loc[i]['future_18_throttle_brake'])))
                    self.actions_future_21.append((float(self.df.loc[i]['future_21_steering_angle']),
                                            float(self.df.loc[i]['future_21_throttle']), 
                                            float(self.df.loc[i]['future_21_brake']),
                                            float(self.df.loc[i]['future_21_throttle_brake'])))
                else:
                    self.actions.append((float(self.df.loc[i]['steering_angle']),
                                            float(self.df.loc[i]['throttle']), 
