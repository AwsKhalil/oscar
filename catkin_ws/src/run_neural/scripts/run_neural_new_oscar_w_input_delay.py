...
from drive_run_new_oscar import DriveRun
...
def main(weight_file_name):
    ...
    while not rospy.is_shutdown():
        ...
        delay_time_trigger_in_secs = 8 * 60 * 60 + 5 * 60 
        input_tuple = (neural_control.image, velocity, acceleration)
        if Config.run_neural['delay'] == True: #and sim_time.to_sec() > delay_time_trigger_in_secs:
            ...
            if len(in_delay_buffer) > Config.run_neural['delta']:
        ...
        # if Config.run_neural['delay'] == False:
        #     # time.sleep(0.1)
        ...
        #     # If delay_buffer size exceeds delta, pop the oldest element
        #     if len(delay_buffer) > Config.run_neural['delta']:
        #         # pop the first element in the array which represents joy_data(t-delta)
